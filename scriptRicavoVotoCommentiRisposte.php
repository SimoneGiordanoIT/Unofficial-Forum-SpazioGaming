...
/*Lo script si occupa di ricavare il voto di un commento di risposta, calcolato come media pesata in cui si tiene conto del voto espresso dall'utente e della reputazione dell'utente che ha sottomesso il voto. A tal scopo
...
l'ID del commento figlio nel file coincide con l'ID del commento di interesse; se il controllo da esito positivo, allora si prendono tutti i voti di quel determinato commento; se il numero di votazioni è diverso da
...
error_reporting(0);
    ...
    for ($p=0; $p<$elementiCOMM->length; $p++) {
      
      $elementoCOMM = $elementiCOMM->item($p);
      ...
      if($ID_POST_RIF==$ID_POST_SPECIFICO){
          ...
          if($ID_COMM_RIF==$ID_Child){
            ...
            if($numValutazioniCommenti != 0){
              ...
              for($x = 0;$x <$numValutazioniCommenti ; $x++){

                $valutazioneComm = $listaValutazioniCommenti->item($x); /* prende la valutazione indicata da item()*/ 
